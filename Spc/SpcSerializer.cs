...
public static class SpcSerializer
{
    ...
    private static byte[] CONST_FILE_MAGIC_BYTE = new byte[4] {0x43,0x50,0x53,0x2E};

    public static bool IsSpc(nint handle)
    {
        Stream stream = new FileStream(new Microsoft.Win32.SafeHandles.SafeFileHandle(handle, false), FileAccess.Read);
        // Read and verify SPC header magic string
        byte[] buf = new byte[CONST_FILE_MAGIC_BYTE.Length];
        stream.Position = 0;
        stream.Read(buf);
        stream.Dispose();
        stream.Close();

        return ( CONST_FILE_MAGIC_BYTE.SequenceEqual(buf));
