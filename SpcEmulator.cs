...
namespace SPC.Stream.Emulator
{
    internal class SpcEmulator : IEmulator
    {
        ...
        public bool TryCreateFile(IntPtr handle, string filepath, string route, out IEmulatedFile emulatedFile)
        {
            ...
            // Check extension.
            if (!filepath.EndsWith(Constants.SpcExtension, StringComparison.OrdinalIgnoreCase))
                return false;


            // Check file type.
            if (!DRV3_Sharp_Library.Formats.Archive.SPC.SpcSerializer.IsSpc(handle)) // WadLib.Wad.IsWad(handle))
                return false;
        ...
        public bool TryCreateEmulatedFile(IntPtr handle, string filepath, string route, ref IEmulatedFile? emulatedFile, out MultiStream? stream)
        {
            ...
            _log.Debug("IS spc: " + DRV3_Sharp_Library.Formats.Archive.SPC.SpcSerializer.IsSpc(handle).ToString());
            if (!DRV3_Sharp_Library.Formats.Archive.SPC.SpcSerializer.IsSpc(handle)) // WadLib.Wad.IsWad(handle))
                return false;
        ...
        private void DumpFile(string filepath, MultiStream stream)
        {
            ...
            _log.Info($"[{nameof(SpcEmulator)}] Dumping {filepath}");
            ...
            fileStream.Close();
            _log.Info($"[{nameof(SpcEmulator)}] Written To {filePath}");
        }
}
}
